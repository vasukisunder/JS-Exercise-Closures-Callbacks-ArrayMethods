...
function processDuplicateFree(list, callback) {
    ...
    const ans =  list.filter((a, b) => list.indexOf(a) === b);
    return ans;
}
...
const ans =[];
  runners.forEach(function(item){
    var string = item.last_name + ", " + item.first_name;
    ans.push(string);
  });
  return ans;
}
...
function firstNamesAllCaps(runners) {
  ...
  const ans = runners.map((item) => item.first_name.toUpperCase());
...
function tallyUpDonations(runners) {
 ...
 const ans = runners.reduce((total, item) => {
   return total + item.donation}, 0);
